...
public interface AisRepository extends JpaRepository<Ais, AisKey> {
    ...
    @Query(value = "select * from (\n" +
            "\tselect ais.signal_date, ais.cog, ais.posX, ais.posY, ais.sog, ais.mmsi, ship.shipType from ais inner join" +
            " ship ship on ais.mmsi = ship.mmsi and ship.shipType = ?1 order by ais.mmsi, signal_date DESC limit 18446744073709551615\n" +
            "    ) as recent_ais group by mmsi", nativeQuery = true)
    ...
    @Query(value = "select * from (select * from ais order by mmsi, signal_date DESC limit 18446744073709551615) as recent_ais" +
                    " group by mmsi having timestampdiff(MINUTE, signal_date, current_timestamp) >= 5", nativeQuery = true)
